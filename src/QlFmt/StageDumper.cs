...
internal static class StageDumper
{
    ...
    public static string DumpTree(string source, Query query)
    {
        ...
        if (query is SetOperation { CorrespondingKeyword: { } corresponding } setOp)
        {
            dump.AppendLine();
            dump.Append(Indent);
            AppendLabeledToken(dump, source, nameof(SetOperation.CorrespondingKeyword), corresponding);
            if (setOp.ByKeyword is { } by)
            {
                dump.AppendLine();
                dump.Append(Indent);
                AppendLabeledToken(dump, source, nameof(SetOperation.ByKeyword), by);
            }
        }
    ...
    private static string DumpSelect(string source, SelectStatement statement)
    {
        ...
        if (statement.AllKeyword is { } all)
        {
            dump.AppendLine();
            dump.Append(Indent);
            AppendLabeledToken(dump, source, nameof(SelectStatement.AllKeyword), all);
        }
        ...
        if (statement.FromKeyword is { } fromKeyword && statement.From is { } from)
        {
            ...
            foreach (var extra in statement.ExtraFrom)
            {
                dump.AppendLine();
                dump.Append(Indent);
                AppendTableSource(dump, source, extra.Table, NestedChildDepth);
            }
        }
    ...
    private static void AppendTableSource(StringBuilder dump, string source, TableSource table, int childDepth)
    {
        switch (table)
        {
            ...
            case JoinedTable joined:
                dump.Append(nameof(JoinedTable));
                dump.AppendLine();
                AppendIndent(dump, childDepth);
                AppendTableSource(dump, source, joined.Table, childDepth + 1);
                break;
            default:
    ...
    private static void AppendExpression(StringBuilder dump, string source, Expression expression, int depth)
    {
        ...
        switch (expression)
        {
            ...
            case CollateExpression collate:
                dump.Append(nameof(CollateExpression));
                dump.AppendLine();
                AppendExpression(dump, source, collate.Expression, depth + 1);
                dump.AppendLine();
                AppendIndent(dump, depth + 1);
                AppendSpan(dump, source, collate.Name);
                break;
            ...
            case NiladicFunctionExpression niladic:
                ...
                dump.Append(' ');
                AppendSpan(dump, source, niladic.Name);
                break;
            ...
            case DatetimeLiteralExpression datetime:
                dump.Append(nameof(DatetimeLiteralExpression));
                dump.Append(' ');
                AppendSpan(dump, source, datetime.KindKeyword);
                dump.Append(' ');
                AppendSpan(dump, source, datetime.Literal);
                break;
            case IntervalLiteralExpression interval:
                dump.Append(nameof(IntervalLiteralExpression));
                dump.Append(' ');
                AppendSpan(dump, source, interval.Literal);
                break;
            case TrimExpression trim:
                dump.Append(nameof(TrimExpression));
                dump.AppendLine();
                AppendExpression(dump, source, trim.Source, depth + 1);
                break;
            case ExtractExpression extract:
                dump.Append(nameof(ExtractExpression));
                dump.AppendLine();
                AppendExpression(dump, source, extract.Source, depth + 1);
                break;
            case SubstringExpression substring:
                dump.Append(nameof(SubstringExpression));
                dump.AppendLine();
                AppendExpression(dump, source, substring.Source, depth + 1);
                break;
            case PositionExpression position:
                dump.Append(nameof(PositionExpression));
                dump.AppendLine();
                AppendExpression(dump, source, position.Needle, depth + 1);
                dump.AppendLine();
                AppendExpression(dump, source, position.Haystack, depth + 1);
                break;
            case UsingTransformExpression transform:
                dump.Append(nameof(UsingTransformExpression));
                dump.AppendLine();
                AppendExpression(dump, source, transform.Expression, depth + 1);
                break;
            ...
            case IsExpression isExpression:
                dump.Append(nameof(IsExpression));
                dump.AppendLine();
                AppendExpression(dump, source, isExpression.Target, depth + 1);
                break;
            case RowConstructorExpression row:
                dump.Append(nameof(RowConstructorExpression));
            ...
            case OverlapsExpression overlaps:
                dump.Append(nameof(OverlapsExpression));
                dump.AppendLine();
                AppendExpression(dump, source, overlaps.Left, depth + 1);
                dump.AppendLine();
                AppendExpression(dump, source, overlaps.Right, depth + 1);
                break;
            ...
            case UniqueExpression unique:
                dump.Append(nameof(UniqueExpression));
                dump.AppendLine();
                AppendIndent(dump, depth + 1);
                AppendQueryHead(dump, source, unique.Query);
                break;
            case MatchExpression match:
                dump.Append(nameof(MatchExpression));
                dump.AppendLine();
                AppendExpression(dump, source, match.Left, depth + 1);
                dump.AppendLine();
                AppendIndent(dump, depth + 1);
                AppendQueryHead(dump, source, match.Query);
                break;
            ...
            case FunctionCallExpression call:
                ...
                {
                    dump.AppendLine();
                    AppendExpression(dump, source, filter.Expression, depth + 1);
                }
